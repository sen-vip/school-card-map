...
- 결과 표를 `일자 / 사용처 / 금액 / 지도상태` 4열 구조로 정리했습니다.
- `집행목적`은 별도 열에서 빼고 사용처명 아래 보조정보로 표시해 표 폭을 확보했습니다.
- 표 안의 날짜는 `2026-02-04` 대신 `02.04` 형태로 짧게 표시합니다.
- 지도상태 배지는 칸 전체를 채우지 않고 내용 길이에 맞는 pill 형태로 정리했습니다.
- `다른 위치 찾기` 버튼 문구를 `위치 수정`으로 줄였습니다.
- 모바일에서는 결과 목록이 표가 아니라 한 건씩 읽기 쉬운 카드형으로 표시됩니다.
