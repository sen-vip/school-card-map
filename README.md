...
> 바로가기: school-card-map.vercel.app/
...
### v1.2.13 검색 진행 점점점 피드백

- 학교 지역 확인, 서울교육청 자료 수집, 지도 대상 정리, 사용처 위치 검색 중에는 상태 문구와 메인 버튼에 움직이는 점점점 로딩 표시가 붙습니다.
- 공개자료 수집이 오래 걸릴 때도 화면이 멈춘 것처럼 보이지 않도록 진행 상태 피드백을 강화했습니다.
- 기존 지도 생성, 위치 수정, 모바일 카드형 목록 기능은 유지됩니다.
...
### v1.3.2 기준월 문구·정렬 정리
- 기준월 아래의 “이번 달 이후는 선택할 수 없습니다.” 고정 안내 문구를 제거했습니다.
- 기준월 선택 제한은 유지하되, 화면은 학교명·기준월·실행 버튼이 같은 선상에서 보이도록 정리했습니다.

### v1.3.1 기준월 선택 제한

- 기준월 기본값은 지난달로 유지합니다.
- 이번 달부터 이후 월은 선택하지 못하도록 `type="month"`의 최대값을 지난달로 제한했습니다.
- 이전 저장값이 이번 달 이후인 경우 접속 시 자동으로 지난달로 보정합니다.
- 공개자료가 아직 올라오지 않은 당월 조회로 사용자가 헷갈리는 상황을 줄입니다.

### v1.3.0 UI/UX 정리·첫 화면 단순화

- 첫 화면에서 학교명, 기준월, 사용처 지도 만들기 버튼이 가장 먼저 보이도록 정리했습니다.
- 결과가 나오기 전에는 0건 요약 카드가 화면을 차지하지 않도록 숨겼습니다.
- 지도와 사용처 카드 목록의 위계를 조정해 결과 화면에서 지도가 더 주인공처럼 보이게 했습니다.
- 설정, 자료 가져오기 상세, 수동 입력, 지도 제외 규칙은 보조 기능처럼 보이도록 시각적 무게를 낮췄습니다.
- 네이비 기반 색상은 유지하되 완료/확인/실패/제외 상태 색을 더 차분한 톤으로 정리했습니다.


### v1.3.3 제목·푸터 정리

- 앱 제목을 `학교카드 사용지도`로 변경했습니다.
- 브라우저 title도 `학교카드 사용지도`로 변경했습니다.
- 푸터에 제작자 `senvip`과 GitHub 링크를 추가했습니다.
